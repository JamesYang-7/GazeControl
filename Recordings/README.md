...
| `Study_02/` | `Study2Scene` | Study 2 (3People-2022 clips, five of `study2_c1`–`c7` in a session). **Each participant's takes live under their own folder**: `P01/session.json`, `responses.csv`, `comments.jsonl`, and `P01/study2_c1/…P01_<condition>_<timestamp>.{csv,json,_user.csv}` per clip (since 2026-09-08; a participant's abandoned first attempt had survived the deletion of their folder in the flat layout). Deleting `P01/` removes everything that participant produced. Take folders directly under `Study_02/` are bakes, which have no participant; a `P00` development run files its takes under its own timestamped `P00_<date>_<time>/` folder beside the answers (since 2026-09-09; before that its takes landed in the flat clip folders). |
...
| `Debug/` | development runs on the `P00` label | One timestamped `P00_<date>_<time>` folder per run, holding whatever the questionnaire wrote and, since 2026-09-09, the run's take logs under `<clip>/`. |
