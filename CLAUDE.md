...
**Still unverified, and the first thing to do next**: nothing has been run through the *Varjo loader* yet — the measurements above are the OpenXR path, and the loader switch came after them. Also open: eye-height calibration has never been run on a real worn headset, nothing reads gaze yet (§7's measures), there is no in-VR questionnaire, and per-participant calibration is not persisted. Note for whoever debugs gaze: Varjo gates eye tracking behind a permission in **Varjo Base** and behind its own per-wearer ET calibration, and some runtimes suppress gaze entirely while the headset is off the head — check all three before suspecting the code.
