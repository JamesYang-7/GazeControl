...
{
    /// <summary>
    /// Adapted from Unity sentis-samples TextToSpeechSample (KokoroHandler.cs).
    /// Changes vs the original: the model and voices load from Assets/Models/
    /// (git-ignored, see the project README) via AssetDatabase instead of
    /// Resources, so the ~310 MB model stays out of git and out of builds; and
    /// voices load individually by name instead of via a voicesIndex resource.
    /// Editor-only as a consequence — for standalone builds, move the model and
    /// voices under a Resources folder and restore the original loading.
    /// </summary>
    public class KokoroHandler : IDisposable
    {
        public const string ModelAssetPath = "Assets/Models/Kokoro-82M-v1.0.onnx";
        public const string VoicesFolder = "Assets/Models/Voices";
...
#if UNITY_EDITOR
            var modelAsset = UnityEditor.AssetDatabase.LoadAssetAtPath<ModelAsset>(ModelAssetPath);
#else
            ModelAsset modelAsset = null;
#endif
            if (modelAsset == null)
                throw new InvalidOperationException(
                    $"Kokoro model not found at '{ModelAssetPath}'. See README.md for the download link (editor-only setup).");
        ...
        /// <summary>Load one voice style by name, e.g. "am_adam" (expects Assets/Models/Voices/am_adam.bin).</summary>
        public static Voice GetVoice(string name)
        {
            var path = $"{VoicesFolder}/{name}.bin";
#if UNITY_EDITOR
            var voiceAsset = UnityEditor.AssetDatabase.LoadAssetAtPath<RawBytesAsset>(path);
#else
            RawBytesAsset voiceAsset = null;
#endif
            if (voiceAsset == null)
                throw new InvalidOperationException(
                    $"Kokoro voice '{name}' not found at '{path}'. See README.md for the download instructions.");
